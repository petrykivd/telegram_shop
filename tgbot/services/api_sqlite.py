...
def next_non_zero_number(initial_number):
    next_number = initial_number + 1
...
def add_userx(user_id, user_login, user_name, ref_id):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        # Получаем текущее максимальное значение из столбца "increment"
        result = con.execute("SELECT MAX(increment) FROM storage_users").fetchone()
        max_increment = result['MAX(increment)'] if result['MAX(increment)'] is not None else 0
        next_increment = next_non_zero_number(max_increment)
        # Вставляем новую запись с увеличенным значением "increment"
        con.execute("INSERT INTO storage_users "
                    "(increment, user_id, user_login, user_name, user_balance, user_refill, user_reff, ref_id, user_date, user_unix) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [next_increment, user_id, user_login, user_name, 0, 0, 0, ref_id, get_date(), get_unix()])
...
# Удаление всех пополнений
def del_all_refillx():
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = "DELETE FROM storage_refill"
        return con.execute(sql).fetchall()
...
# Добавление категории
def add_positionx(position_id, position_name, position_price, position_description, position_photo, category_id, single_item):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO storage_position "
                    "(position_id, position_name, position_price, position_description, position_photo, position_date, category_id, single_item) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [position_id, position_name, position_price, position_description,
                     position_photo, get_date(), category_id, single_item])
...
def get_positionsx(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        sql = "SELECT * FROM storage_position"
        sql, parameters = update_format_where(sql, kwargs)
        sql += " ORDER BY position_price DESC"
        return con.execute(sql, parameters).fetchall()
    ...
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        if len(con.execute("PRAGMA table_info(storage_payment)").fetchall()) == 7:
            ...
            con.execute("INSERT INTO storage_payment("
                        "binance_login, binance_token, binance_secret, usdt_trc20, way_binance, way_usdt_trc20, way_monobank) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        if len(con.execute("PRAGMA table_info(storage_position)").fetchall()) == 9:
