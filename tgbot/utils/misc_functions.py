...
from tgbot.services.api_sqlite import (get_settingsx, update_settingsx, get_userx, get_purchasesx, get_all_positionsx,
                                       ...
                                       get_itemsx, get_positionx, get_categoryx, get_paymentx,
                                       add_refillx,get_refillx, update_userx
                                       )
...
async def send_admins(message, markup=None, not_me=0):
    try:
        await bot.send_message("-1001949289144", message, disable_web_page_preview=True)
    except:
        pass
...
async def apply_payment(client,amount,receipt,get_way):
    amount = float(amount)
    get_refill = get_refillx(refill_receipt=receipt)
    if get_refill is None:
        get_user = get_userx(increment=client)
        print(f"\nUSER:\n{get_user}\n")
        ...
        await bot.send_message(get_user['user_id'],
...
async def check_wallets():
    ...
    if get_payments['way_binance'] == "True":
        client = Client(get_paymentx()['binance_token'],get_paymentx()['binance_secret'])
        current_time = int(time.time())
        one_hour_ago = current_time - 3600
        try:
            for transaction in client.pay_history(startTimestamp=one_hour_ago, limit=50)['data']:
                if transaction['orderType'] == 'C2C' and transaction['note'] and transaction['currency'] == 'USDT':
                    transact_time = int(transaction['transactionTime']) // 1000  # преобразование времени в секунды
                    #if current_time - transact_time <= 3600:
                    if is_convertible_to_integer(transaction['note']):
                        await apply_payment(
                            client = int(transaction['note']),
                            receipt = transaction['orderId'],
                            amount = transaction['amount'],
                            get_way = "Binance"
                    )
        except Exception as ex:
            print(f"BinancePay check error: {ex}")
    if get_payments['way_usdt_trc20'] == "True":
        try:
            url = f"https://api.trongrid.io/v1/accounts/{get_paymentx()['usdt_trc20']}/transactions/trc20?limit=100&only_confirmed=true&contract_address=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"
            response = requests.get(url)
            for i in response.json()['data']:
                value = float(i['value']) / (10 ** i['token_info']['decimals'])
                amount = str(value).split('.')
                if i['to'] == get_paymentx()['usdt_trc20']:
                    await apply_payment(
                            client = int(amount[1]),
                            receipt = i['transaction_id'],
                            amount = value,
                            get_way = "USDT"
                        )
        except Exception as ex:
            print(f"USDT-TRC20 check error: {ex}")
    if get_payments['way_monobank'] == "True":
        pass
