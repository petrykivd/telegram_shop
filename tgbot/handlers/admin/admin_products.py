...
from aiogram.types import CallbackQuery, Message,Document, ContentType
...
async def product_position_create_select_category(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(here_cache_single_item_state=single_item)
...
async def product_position_create_photo(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        single_item_state = data['here_cache_single_item_state']
    ...
    add_positionx(position_id, position_name, position_price, position_description, position_photo, category_id, single_item_state)
    get_message, get_photo = get_position_admin(position_id)
    
    if(single_item_state):
        ...
        await message.answer("<b>📤 Отправьте данные товара в .txt документе.</b>\n")
    else:
        await state.finish()
...
async def product_item_position_open(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state("here_add_items")
    await call.message.answer(
        "<b>📤 Отправьте данные товаров в .txt документе.</b>\n"
...
@rate_limit(0)
@dp.message_handler(IsAdmin(), state="here_add_items", content_types=[ContentType.DOCUMENT])
async def product_item_load_get(message: Message, state: FSMContext):
    if message.document:
        ...
        if document.mime_type == 'text/plain' and document.file_name.endswith('.txt'):
            file = await document.download(destination="/home/bot2")  # Путь сохранения на диск
            with open(file.name, 'r', encoding='utf-8') as txt_file:
                file_contents = txt_file.read()
            os.remove(file.name)
        await message.answer("Текстовый файл принят. Обрабатываем данные...")
        
    cache_msg = await message.answer("<b>⌛ Ждите, товары добавляются...</b>")

    count_add = 0
    get_all_items = clear_list(file_contents.split("\n"))
