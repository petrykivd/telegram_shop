...
from tgbot.keyboards.inline_user import refill_bill_finl, refill_select_finl, refill_keyboard_check
...
min_input_qiwi = 2  # Минимальная сумма пополнения в рублях
...
async def refill_get(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if min_input_qiwi <= pay_amount <= 300000:
            ...
            ).bill(pay_amount, get_way, get_userx(user_id=message.from_user.id)['increment'])
            
            if get_message:
                await cache_message.edit_text(get_message, reply_markup=refill_keyboard_check())
        else:
            await cache_message.edit_text(
                f"<b>❌ Неверная сумма пополнения</b>\n"
                f"▶ Cумма не должна быть меньше <code>{min_input_qiwi}$</code> и больше <code>30 000$</code>\n"
...
async def refill_check_form(call: CallbackQuery):
    await call.answer("❗ Ожидайте сообщение об успешном пополнении, при правильном отправлении все происходит автоматически.", True)
