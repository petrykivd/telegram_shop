...
# Открытие профиля
@dp.callback_query_handler(text="user_reff", state="*")
async def user_reff(call: CallbackQuery, state: FSMContext):
    user_id = call.from_user.id
    get_user = get_userx(user_id=user_id)
    reffs = len(get_usersx(ref_id=user_id))
    await call.message.answer(
        ded(f"""
                👬 <b>Реферальная программа</b>

                Пригласите друзей через вашу реферальную ссылку, и они станут вашими рефералами. Каждая их покупка приносит вам проценты на баланс!

                🛎 Ссылка: https://t.me/ADD_FUNCTION_HERE?start={call.from_user.id}
                🔥 Ваш заработок: 5% от суммы покупки

                ➖➖➖➖➖➖➖➖➖➖➖

                🎖 Рефералы: {reffs} чел.
                🎁 Заработок: {get_user['user_reff']} $
            """)
        )
...
async def user_purchase_select(call: CallbackQuery, state: FSMContext):
    ...
    if get_position['position_price'] != 0:
        get_count = float(get_user['user_balance'] / get_position['position_price'])
        if get_count > len(get_items): get_count = len(get_items)
    else:
        get_count = len(get_items)

    if float(get_user['user_balance']) >= float(get_position['position_price']):
        if get_count == 1:
    ...
    if message.text.isdigit():
        ...
        if len(get_items) >= 1:
            if 1 <= get_count <= len(get_items):
                if float(get_user['user_balance']) >= amount_pay:
                    await state.finish()
                    await message.answer(
                        ded(f"""
                            <b>🎁 Вы действительно хотите купить товар(ы)?</b>
    ...
    if get_action == "yes":
        ...
        if 1 <= int(get_count) <= len(get_items):
            if get_user['user_balance'] >= amount_pay:
                ...
                print(get_user['ref_id'])
                if(get_user['ref_id']):
                    refer = get_userx(user_id=get_user['ref_id'])

                    update_userx(refer['user_id'], user_balance=refer['user_balance'] + round(amount_pay*0.05, 2), 
                                                    user_reff=round(amount_pay*0.05, 2)
                    )
                ...
                """
                file_content = "\n".join(save_items).encode("utf-8")

                # Создание временного файла в памяти
                file_obj = BytesIO(file_content)
                file_obj.name = "thanks.txt"
