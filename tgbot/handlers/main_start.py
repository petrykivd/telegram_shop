...
from tgbot.keyboards.inline_user import user_support_finl,sxema_keyboard
...
prohibit_refill = ['user_refill', 'refill_select', 'Pay:', 'Pay:Binance', 'Pay:USDT', 'Pay:Monobank']
...
@dp.message_handler(text=['СХЕМА'], state="*")
async def sxema_start(message: Message, state: FSMContext):
    await state.finish()
