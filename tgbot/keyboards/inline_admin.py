...
def payment_choice_finl():
    ...
    status_form_kb = ikb("✅", callback_data="change_payment:Binance:False")
    status_number_kb = ikb("✅", callback_data="change_payment:USDT:False")
    #status_nickname_kb = ikb("✅", callback_data="change_payment:Monobank:False")

    if get_payments['way_binance'] == "False":
        status_form_kb = ikb("❌", callback_data="change_payment:Binance:True")
    if get_payments['way_usdt_trc20'] == "False":
        status_number_kb = ikb("❌", callback_data="change_payment:USDT:True")
    #if get_payments['way_monobank'] == "False":
    #    status_nickname_kb = ikb("❌", callback_data="change_payment:Monobank:True")

    keyboard.add(
        ikb("📋 По Binance Pay",url="https://bem.bem/"), status_form_kb
    ).row(
        ikb("📞 По USDT-TRC20",url="https://bem.bem/"), status_number_kb
    )
    #.row(
    #    ikb("Ⓜ По Monobank", url="https://vk.cc/c8s66X"), status_nickname_kb
    #)
...
def category_edit_open_finl(category_id, remover):
    ...
    keyboard.add(
        ...
        ikb("📁 Одиночный товар", callback_data=f"position_create_open:{category_id}:1")
    ).add(
