...
def refill_select_finl():
    ...
    if get_payments['way_binance'] == "True":
        active_kb.append(InlineKeyboardButton("📋 Binance Pay", callback_data="refill_select:Binance"))
    if get_payments['way_usdt_trc20'] == "True":
        active_kb.append(InlineKeyboardButton("📞 USDT TRC-20", callback_data="refill_select:USDT"))
    if get_payments['way_monobank'] == "True":
        active_kb.append(InlineKeyboardButton("Ⓜ Monobank", callback_data="refill_select:Monobank"))
...
def refill_bill_finl(send_requests, get_receipt, get_way):
    ...
    return keyboard
def refill_keyboard_check():
    keyboard = InlineKeyboardMarkup()
    keyboard.add(
        InlineKeyboardButton("🔄 Проверить оплату", callback_data=f"Pay:None:None")
        )
    return keyboard
...
def sxema_keyboard():
    ...
    keyboard.add(
        InlineKeyboardButton("СХЕМА", url=f"https://youtu.be/VWXIXJGAIFY"),
    )
